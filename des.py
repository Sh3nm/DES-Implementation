# Konversi string ke bit
def text_to_bits(text):
    return ''.join(f'{ord(c):08b}' for c in text)

# Konversi bit ke string
def bits_to_text(bits):
    chars = [bits[i:i+8] for i in range(0, len(bits), 8)]
    return ''.join(chr(int(b, 2)) for b in chars)

def xor(a, b):
    return ''.join('1' if i != j else '0' for i, j in zip(a, b))

def permute(bits, table):
    return ''.join(bits[i-1] for i in table)

def pkcs7_pad(data_bytes, block_size=8):
    pad_len = block_size - (len(data_bytes) % block_size)
    return data_bytes + bytes([pad_len]) * pad_len

def pkcs7_unpad(padded_bytes):
    pad_len = padded_bytes[-1]
    if pad_len < 1 or pad_len > 8:
        raise ValueError("Invalid padding")
    return padded_bytes[:-pad_len]


# Tambahkan padding agar panjang kelipatan 64
def pad_to_64_bits(bits):
    while len(bits) % 64 != 0:
        bits += '0'
    return bits

# table DES
IP = [58, 50, 42, 34, 26, 18, 10, 2,
      60, 52, 44, 36, 28, 20, 12, 4,
      62, 54, 46, 38, 30, 22, 14, 6,
      64, 56, 48, 40, 32, 24, 16, 8,
      57, 49, 41, 33, 25, 17, 9,  1,
      59, 51, 43, 35, 27, 19, 11, 3,
      61, 53, 45, 37, 29, 21, 13, 5,
      63, 55, 47, 39, 31, 23, 15, 7]

FP = [40, 8, 48, 16, 56, 24, 64, 32,
      39, 7, 47, 15, 55, 23, 63, 31,
      38, 6, 46, 14, 54, 22, 62, 30,
      37, 5, 45, 13, 53, 21, 61, 29,
      36, 4, 44, 12, 52, 20, 60, 28,
      35, 3, 43, 11, 51, 19, 59, 27,
      34, 2, 42, 10, 50, 18, 58, 26,
      33, 1, 41, 9, 49, 17, 57, 25]

E = [32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9,
     8, 9, 10, 11, 12, 13, 12, 13, 14, 15, 16, 17,
     16, 17, 18, 19, 20, 21, 20, 21, 22, 23, 24, 25,
     24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1]

P_BOX_TABLE = [
    15, 6, 19, 20, 28, 11, 27, 16,
    0, 14, 22, 25, 4, 17, 30, 9,
    1, 7, 23, 13, 31, 26, 2, 8,
    18, 12, 29, 5, 21, 10, 3, 24
]

# S-box (pake 1 contoh dulu buat semuanya)
S_BOX = [
    [
        [14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7],
        [0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8],
        [4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0],
        [15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13]
    ],
    [
        [15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10],
        [3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5],
        [0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15],
        [13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9]
    ],
    [
        [10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8],
        [13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1],
        [13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7],
        [1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12]
    ],
    [
        [7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15],
        [13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9],
        [10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4],
        [3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14]
    ],
    [
        [2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9],
        [14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6],
        [4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14],
        [11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3]
    ],
    [
        [12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11],
        [10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8],
        [9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6],
        [4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13]
    ],
    [
        [4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1],
        [13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6],
        [1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2],
        [6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12]
    ],
    [
        [13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7],
        [1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2],
        [7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3, 5, 8],
        [2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11]
    ]
]

def sbox_substitution(bits):
    output = ''
    for i in range(8):
        block = bits[i*6:(i+1)*6]
        row = int(block[0] + block[5], 2)
        col = int(block[1:5], 2)
        val = S_BOX[i][row][col]
        output += f'{val:04b}'
    return output

def feistel(right, key):
    # Expansion dari 32 ke 48 bit
    expanded = permute(right, E)

    # XOR
    temp = xor(expanded, key)

    # S-box dari 48 ke 32
    substituted = sbox_substitution(temp)

    # P-box permuatation
    permuted = permute(substituted, P_BOX_TABLE)
    return permuted  

# Hasilkan 16 subkey dari key
PC1 = [57,49,41,33,25,17,9, 1,58,50,42,34,26,18, 10,2,59,51,43,35,27, 19,11,3,60,52,44,36,
       63,55,47,39,31,23,15,7,62,54,46,38,30,22,14,6,61,53,45,37,29,21,13,5,28,20,12,4]
PC2 = [14,17,11,24,1,5, 3,28,15,6,21,10, 23,19,12,4,26,8, 16,7,27,20,13,2,
       41,52,31,37,47,55,30,40,51,45,33,48, 44,49,39,56,34,53,46,42,50,36,29,32]
SHIFTS = [1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1]

def generate_subkeys(key_bits):
    key56 = permute(key_bits, PC1)   # apply PC-1
    C, D = key56[:28], key56[28:]
    subkeys = []
    for s in SHIFTS:
        C = C[s:] + C[:s]
        D = D[s:] + D[:s]
        subkeys.append(permute(C + D, PC2))
    return subkeys


# DES Core

# Ennkripsi
def des_encrypt_block(block, key_bits):
    block = permute(block, IP)
    L, R = block[:32], block[32:]
    subkeys = generate_subkeys(key_bits)

    for i in range(16):
        new_L = R
        new_R = xor(L, feistel(R, subkeys[i]))
        L, R = new_L, new_R

    cipher_block = permute(R + L, FP)
    return cipher_block

# Enkripsi untuk ECB
def des_encrypt_ecb(plaintext, key):
    data_bytes = pkcs7_pad(plaintext.encode('utf-8'))
    key_bits = text_to_bits(key)
    cipher_bits = ''

    for i in range(0, len(data_bytes), 8):
        block_bits = ''.join(f'{b:08b}' for b in data_bytes[i:i+8])
        cipher_bits += des_encrypt_block(block_bits, key_bits)

    return hex(int(cipher_bits, 2))[2:].zfill(len(cipher_bits)//4)

# Enkripsi untuk CBC
def des_encrypt_cbc(plaintext, key, iv="12345678"):
    data_bytes = pkcs7_pad(plaintext.encode('utf-8'))
    key_bits = text_to_bits(key)
    iv_bits = text_to_bits(iv)
    cipher_bits = ''
    prev = iv_bits

    for i in range(0, len(data_bytes), 8):
        block_bits = ''.join(f'{b:08b}' for b in data_bytes[i:i+8])
        # XOR dengan IV atau cipher sebelumnya
        xor_input = xor(block_bits, prev)
        encrypted = des_encrypt_block(xor_input, key_bits)
        cipher_bits += encrypted
        prev = encrypted  # chaining

    return hex(int(cipher_bits, 2))[2:].zfill(len(cipher_bits)//4)

# Dekripsi
def des_decrypt_block(block, key_bits):
    block = permute(block, IP)
    L, R = block[:32], block[32:]
    subkeys = generate_subkeys(key_bits)

    # urutan subkeys dibalik
    for i in range(15, -1, -1):
        new_L = R
        new_R = xor(L, feistel(R, subkeys[i]))
        L, R = new_L, new_R

    return permute(R + L, FP)

# Dekripsi untuk ECB
def des_decrypt_ecb(cipher_hex, key):
    key_bits = text_to_bits(key)
    cipher_bits = bin(int(cipher_hex, 16))[2:].zfill(len(cipher_hex) * 4)
    decrypted_bytes = bytearray()

    for i in range(0, len(cipher_bits), 64):
        block_bits = cipher_bits[i:i+64]
        plain_bits = des_decrypt_block(block_bits, key_bits)
        for j in range(0, 64, 8):
            decrypted_bytes.append(int(plain_bits[j:j+8], 2))

    return pkcs7_unpad(bytes(decrypted_bytes)).decode('utf-8')

# Dekripsi untuk CBC
def des_decrypt_cbc(cipher_hex, key, iv="12345678"):
    key_bits = text_to_bits(key)
    iv_bits = text_to_bits(iv)
    cipher_bits = bin(int(cipher_hex, 16))[2:].zfill(len(cipher_hex) * 4)
    decrypted_bytes = bytearray()
    prev = iv_bits

    for i in range(0, len(cipher_bits), 64):
        block_bits = cipher_bits[i:i+64]
        decrypted = des_decrypt_block(block_bits, key_bits)
        plain_block = xor(decrypted, prev)
        prev = block_bits
        for j in range(0, 64, 8):
            decrypted_bytes.append(int(plain_block[j:j+8], 2))

    return pkcs7_unpad(bytes(decrypted_bytes)).decode('utf-8')

if __name__ == "__main__":
    plaintext = input("Masukkan plaintext: ")
    key = input("Masukkan key (8 karakter): ")
    mode = input("Pilih mode (ecb/cbc): ").lower()

    if mode == "ecb":
        cipher = des_encrypt_ecb(plaintext, key)
        print("Ciphertext (ECB):", cipher)
        decrypted = des_decrypt_ecb(cipher, key)
    elif mode == "cbc":
        cipher = des_encrypt_cbc(plaintext, key)
        print("Ciphertext (CBC):", cipher)
        decrypted = des_decrypt_cbc(cipher, key)
    else:
        print("Mode tidak dikenal.")
        exit()

    print("Hasil dekripsi:", decrypted)


